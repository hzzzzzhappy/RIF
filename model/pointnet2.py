...
from util_.set_abstraction import PointNet_SA_Module, PointNet_SA_Module_MSG
...
class pointnet2_cls_msg(nn.Module):
    def __init__(self, in_channels=3, nclasses=40):
    ...
    def feature_(self, xyz, points=None):
        xyz = xyz.transpose(2, 1)
        batchsize = xyz.shape[0]
        new_xyz, new_points = self.pt_sa1(xyz, points)
        new_xyz, new_points = self.pt_sa2(new_xyz, new_points)
        new_xyz, new_points = self.pt_sa3(new_xyz, new_points)
        new_points = new_points.squeeze()  # 去掉第2个维度 (索引从0开始)
        return new_points
