#!/usr/bin/env python
# -*- coding: utf-8 -*-
from util_.trans import process_single_point_cloud
import os
import argparse
import torch
import numpy as np
...
def test(args, io):
    ...
    # 加载模型并加载预训练权重
    model = GFCNN().to(device) if args.model == 'GFCNN' else PointNet(args).to(device)
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load(f'model.t7'))
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='GFCNN', metavar='N',
                        choices=['pointnet', 'GFCNN'],
