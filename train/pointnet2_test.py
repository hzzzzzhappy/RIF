...
def test(args, io):
    ...
    # 加载模型并加载预训练权重
    model = pointnet2_cls_msg().to(device) if args.model == 'pointnet2_cls_msg' else PointNet(args).to(device)
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load(f'pretrained/pointnet2.t7'))
    model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='pointnet2_cls_msg', metavar='N',
