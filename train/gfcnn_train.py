    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, label) in enumerate(train_loader):
            ...
            if torch.isnan(data).any() or torch.isinf(data).any():
                print(f"input data at batch {batch_idx}")
                continue
            ...
            if torch.isnan(logits).any() or torch.isinf(logits).any():
                print(f"model output at batch {batch_idx}")
                continue
        ...
        if test_acc >= best_test_acc:
            ...
            torch.save(model.state_dict(), 'checkpoints/%s/models/gfcnn.t7' % args.exp_name)
def test(args, io):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='gf', metavar='N',
                        choices=['pointnet', 'gf'],
                        help='Model to use, [pointnet, gf]')
    ...
    parser.add_argument('--num_points', type=int, default=384,
